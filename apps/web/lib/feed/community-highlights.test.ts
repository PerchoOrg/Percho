...
import { COMMUNITY_HIGHLIGHT_COUNT, communityHighlightDims } from './community-highlights';
...
describe('communityHighlightDims', () => {
  it('returns at most the three tiles the redline draws', () => {
    const dims = communityHighlightDims({
      attributes: ['Peaceful', 'Family Friendly', 'Walkability', 'Schools', 'Trees'],
    });
  ...
  it('returns nothing rather than inventing highlights', () => {
    ...
    expect(communityHighlightDims({ attributes: null, interests: null })).toEqual([]);
  ...
  it('tolerates the trailing whitespace the seed carries', () => {
    ...
    expect(communityHighlightDims({ attributes: ['Peaceful '] })).toEqual(['quiet']);
  });

  it('skips non-string entries without throwing', () => {
    expect(
      communityHighlightDims({
        attributes: [null as unknown as string, 42 as unknown as string, 'Quiet'],
      }),
    ).toEqual(['quiet']);
  });
});
...
describe('projectCommunityPool highlight tiles', () => {
  ...
  it('OMITS dims rather than sending [] when there is no signal', () => {
    ...
    const dtos = projectCommunityPool([{ ...row, attributes: null, interests: null }]);
    expect(dtos).toHaveLength(1);
  ...
  it('OMITS reasons rather than sending [] when nothing maps', () => {
    const dtos = projectCommunityPool([{ ...row, attributes: ['Lash Strips'], interests: null }]);
