...
describe('listingHighlightDims', () => {
  ...
  it('orders by PRIORITY, not by order of appearance in the prose', () => {
    expect(
      listingHighlightDims(['Move-in ready. Private backyard. Award-winning schools.']),
  ...
  it('does not claim Top Schools from a bare school-district mention', () => {
    ...
    expect(listingHighlightDims(['Located in the Fulton County school district.'])).toEqual([]);
  });

  it("does not claim a private backyard from the word 'yard' alone", () => {
    expect(listingHighlightDims(['The home has a yard and a driveway.'])).toEqual([]);
  });
  ...
  it('does not match a dim word buried inside another word', () => {
    expect(listingHighlightDims(['The sidewalkable pattern is unusual.'])).not.toContain(
      'walkable',
    );
  });

  it('is case-insensitive — agents write chunks in ALL CAPS', () => {
    expect(listingHighlightDims(['LEVEL BACKYARD and BRAND NEW construction.'])).toEqual([
      'outdoors',
      'move_in',
    ]);
  });
  ...
  it('reads across all paragraphs, not just the first', () => {
    expect(listingHighlightDims(['A lovely home.', 'Quiet cul-de-sac location.'])).toEqual([
      'quiet',
    ]);
  });
  ...
  it('counts a screened porch as outdoor living space', () => {
    expect(listingHighlightDims(['Enjoy the large screened porch out back.'])).toContain(
      'outdoors',
    );
  });
  ...
  it('counts new roof / new HVAC as move-in ready', () => {
    expect(listingHighlightDims(['Major updates done: new roof and new HVAC.'])).toContain(
      'move_in',
    );
  });

  it('counts walk-to-school as a walkability claim', () => {
    expect(listingHighlightDims(['Walk to schools and the park from your door.'])).toContain(
      'walkable',
    );
  });

  it('counts a nature preserve as trails', () => {
    expect(listingHighlightDims(['Minutes from the Blue Heron Nature Preserve.'])).toContain(
      'trails',
    );
  });
  ...
  it('does NOT claim move-in ready from a freshly painted FENCE', () => {
    ...
    expect(
      listingHighlightDims(['A back deck overlooking a freshly painted backyard fence.']),
    ).not.toContain('move_in');
  });

  it('still claims move-in ready from freshly painted interiors', () => {
    expect(listingHighlightDims(['Freshly painted inside with new flooring.'])).toContain(
      'move_in',
    );
  });
});
