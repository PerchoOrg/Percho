...
describe('scoreNeighborhood', () => {
  ...
  it('keeps the four dimensions in a fixed order', () => {
    ...
    expect(dims.map((d) => d.label)).toEqual(['Safety', 'Schools', 'Convenience', 'Potential']);
  });
  ...
  it('credits an unrated POI only when it is genuinely close', () => {
    ...
    const distantScore = distant.dims.find((d) => d.key === 'schools')?.score ?? 0;
  ...
  it('caps a dimension at 10', () => {
    ...
    expect(scoreNeighborhood(pois).dims.find((d) => d.key === 'schools')?.score).toBe(10);
  });
