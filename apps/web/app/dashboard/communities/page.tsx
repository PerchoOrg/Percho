 ...
 *
 * per-row + Add video / Edit semantics; communities globally readable.
 * / 43.10: list → grid.
 * reuse the canonical CommunityGrid for parity.
 * wraps in shared GridPageShell so /dashboard/communities
 ...
 *
 * Phase C (2026-06-24): perf rewrite.
 ...
 *
 * Timing instrumentation is kept for one more deploy so
...
export default async function CommunitiesListPage() {
  ...
  // Supabase round-trip.
  const [sessionRes, agentId] = await Promise.all([supabase.auth.getSession(), getViewerAgentId()]);
  t.mark('auth');

  if (!sessionRes.data.session) redirect('/login?redirect=%2Fdashboard%2Fcommunities');

  // "My neighborhoods" is agent-scoped.
