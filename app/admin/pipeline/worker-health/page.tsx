...
export default async function WorkerHealthPage() {
  ...
  const stalled = h.pending > 0 && (!h.lastReady?.updated_at || Date.now() - new Date(h.lastReady.updated_at).getTime() > 30 * 60 * 1000);
    ...
    <div className="space-y-4">
      <header>
        ...
        <p className="text-ink2 mt-1 text-sm">
          ...
          <code className="font-mono text-xs">worker_heartbeats</code> table will replace this with a
          direct signal from the systemd unit `percho-render-worker`.
        </p>
      </header>

      {stalled && (
        <div className="rounded-2xl border border-red-500/40 bg-red-500/10 p-4 text-sm text-red-500">
          ⚠ {h.pending} pending job(s) with no successful render in the last 30 minutes — worker
          may be stalled. Check <code>journalctl -u percho-render-worker -n 100</code>.
        </div>
      )}
