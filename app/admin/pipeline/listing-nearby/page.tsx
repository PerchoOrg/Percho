/**
 * /admin/pipeline/listing-nearby — table of every listing, with a
 * one-click into the per-listing nearby review panel. Filters bias toward
 * "no community" (the ones that most need this pipeline).
 */
...
async function loadData(filter: string) {
  ...
  const { data: listings } = (await q) as { data: ListingRow[] | null };
  const rows = listings ?? [];

  // Aggregate listing-scoped bucket videos.
  ...
  if (ids.length > 0) {
    ...
    for (const r of gv ?? []) {
      const s =
        statsMap.get(r.listing_id) ??
        ({ listing_id: r.listing_id, ready: 0, pending: 0, failed: 0 } as ScopeStat);
  ...
  return rows.map((r) => ({
    ...r,
    stats: statsMap.get(r.id) ?? { listing_id: r.id, ready: 0, pending: 0, failed: 0 },
  }));
}
...
export default async function ListingNearbyIndex({
  ...
  return (
    <div className="space-y-4">
      <header>
        <h1 className="text-2xl font-semibold">Listing Nearby</h1>
        <p className="text-ink2 mt-1 text-sm">
          Per-listing POI discovery + bucket video generation for listings that don't sit inside a
          curated community. POIs and photo binaries stay global — deduped across the platform.
        </p>
      </header>
