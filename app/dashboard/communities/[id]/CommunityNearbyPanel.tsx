...
export function CommunityNearbyPanel({
  ...
  // ── actions ─────────────────────────────────────────────────────────────
  const refresh = async () => {
    const fresh = await loadNearbyPoisForCommunity(communityId);
    setPois(fresh);
  };
  ...
  const handlePhotoDecision = (poiPhotoId: string, approved: boolean) => {
    ...
    let prevSnapshot: NearbyPoiForCommunity[] | null = null;
    setPois((current) => {
    ...
    void (async () => {
      try {
        await setCommunityPhotoStatus(communityId, poiPhotoId, nextStatus);
      } catch (err) {
  ...
  return (
    <div className="space-y-8">
      ...
      <GeneratedVideosSection communityId={communityId} />
...
function PhotoReviewGrid({
  ...
  photos: NearbyPoiForCommunity['photos'];
  storageBase: string;
  bucket: string;
  onPhotoDecide: (poiPhotoId: string, approved: boolean) => void;
  busy: boolean;
...
function GeneratedVideosSection({ communityId }: { communityId: string }) {
  return (
    <div>
      <div className="mb-3">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {BUCKET_ORDER.map((bucket) => (
          <BucketVideoCard key={bucket} communityId={communityId} bucket={bucket} />
        ))}
      </div>
    </div>
  );
...
function BucketVideoCard({
  ...
  const [status, setStatus] = useState<CommunityBucketVideoStatus>(null);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const [next, elig] = await Promise.all([
        getCommunityBucketVideoStatus(communityId, bucket),
        getCommunityBucketEligiblePhotoCount(communityId, bucket),
      ]);
      if (!cancelled) {
        setStatus(next);
  ...
  const handleGenerate = async () => {
    ...
    setErr(null);
    try {
      const res = await generateCommunityBucketVideo(communityId, bucket);
      if (!res.ok) {
        setErr(res.message);
        return;
      }
      ...
      const t = setInterval(async () => {
        const cur = await getCommunityBucketVideoStatus(communityId, bucket);
        setStatus(cur);
  ...
  const handleRegenerateNarrative = async () => {
    ...
    try {
      const res = await regenerateCommunityBucketVideoNarrative(status.video_id);
      if (!res.ok) {
