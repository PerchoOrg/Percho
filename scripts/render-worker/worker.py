...
def initial_listing_verdicts(
    ...
    for r in records:
        if (r.get("review_status") or "pending") != "pending":
            continue
        tags = fresh.get(r["id"]) or r.get("cached_ai_tags")
...
def process_tag_job(job: dict[str, Any]) -> None:
    ...
    try:
        ...
        written = 0
        errored = 0
        fresh_tags: dict[str, dict[str, Any]] = {}
...
def pick_clip(candidates: list[dict[str, Any]], planned: str | None) -> dict[str, Any] | None:
    """Which of a photo's ready clips goes in the film.

    AI-FIRST. A ready Seedance clip wins over whatever the shot list declared
    (owner 2026-08-17 "有ai 选ai", restated 2026-08-21 "we should give seedance
    higher priority in any case"). Seedance is the only engine that costs money
    and the only one whose output cannot be reproduced, so a generated clip
    must never sit unused because the plan has since settled on DepthFlow for
    that photo.

    Then the planned engine, then any ready row — a shot planned for DepthFlow
    still renders from the Ken Burns clip the photo already has rather than
    dropping out of the film.

    Pure and extracted because the home-tour assembler got this order backwards
    while the community one had it right, and the difference was invisible
    until a paid clip went missing from a finished film.
    """
    if not candidates:
        return None
    return (
    ...
    try:
        ...
        for i, c in enumerate(ordered):
            ...
            if not candidates:
                ...
                continue
            # AI-FIRST, matching the community assembly since 2026-08-17
            # (owner: "有ai 选ai", and again 2026-08-21: "we should give
            # seedance higher priority in any case").
            #
            # A ready Seedance clip wins over whatever the shot list declared.
            # This version preferred the planned engine, which meant a paid
            # clip generated by hand sat unused whenever the plan had since
            # settled on DepthFlow for that photo — the one engine whose output
            # must never be wasted was the one most easily ignored.
            #
            # Then the planned engine, then any ready row: a shot planned for
            # DepthFlow still renders from the Ken Burns clip the photo already
            # has rather than dropping out of the film.
            chosen = pick_clip(candidates, c.get("engine"))
            if chosen is None:
                skipped.append(str(pid))
                continue
