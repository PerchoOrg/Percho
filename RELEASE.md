...
- **The sound and save buttons on a card look like they belong there.** They now sit in one small pill the same height and colour as the LISTING label opposite them, instead of a taller, paler capsule, and both icons are properly drawn at last — the speaker in particular used to smudge into a blob at that size. The save mark is the same bookmark the Saved tab uses, and it fills in green once a home is saved
