...
The four nothing cites any more — `vibe/`, `flipbook-demo/`,
`competitive-map/`, `subdivision-page-v1/` — moved to
[`docs/archive/prototypes/`](../archive/prototypes/) on 2026-09-06.
...
`flipbook-demo/prepare.py` (now under `docs/archive/prototypes/`) had a
Supabase **service-role key hardcoded on line 11**, pointed at the production
project. It sat in plaintext in an untracked folder from July 2026 until
phase53. It now reads `os.environ["SUPABASE_SERVICE_ROLE_KEY"]` like the rest
of the repo.
