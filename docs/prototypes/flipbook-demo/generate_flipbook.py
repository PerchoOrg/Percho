...
def concat_with_crossfade(clips: list[str], dst: str, xfade: float, w: int, h: int, transition: str = "fade") -> float:
    ...
    for i in range(1, len(clips)):
        ...
        out_label = f"v{i}"
        filter_parts.append(
            f"[{prev}][{i}:v]xfade=transition={transition}:duration={xfade}:offset={offset:.3f}[{out_label}]"
        )
        prev = out_label
...
def main() -> None:
    ...
    p.add_argument("--transition", default="fade", help="xfade transition name (fade|slideleft|wipeleft|smoothleft)")
    ...
    with tempfile.TemporaryDirectory(prefix="kenburns-") as tmp:
        ...
        total_dur = concat_with_crossfade(clips, concat_out, xfade, w, h, transition=args.transition)
